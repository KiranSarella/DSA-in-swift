    ...
    2. iterate (lowerBound..<n) 
        ...
        2. insert lowerBound value with matches count
            ...
            if mathces are more then zero
                ...
                - check prefix value can be place at lowerBound lowerBound 
...
func generateZ(_ input: String) -> [Int] {
    ...
    // for lowerBound in 1..<n {
        ...
        print("------> h:\(prefixHead), c:\(lowerBound), [\(lowerBound): \(upperBound)]")
        print((0..<n).map {"\($0)"})
        print(input)
        print(z.map{ "\($0)"})
        ...
        if lowerBound > upperBound {
            ...
            // check if previously computed value can be fit within the lowerBound boundary
            if z[prefixHead] < upperBound - lowerBound + 1 {
                print("USE COMPUTED")
                z[lowerBound] = z[prefixHead]
                print("z[\(lowerBound)] = z[\(prefixHead)]")
            } else {
    ...
    //     // insert matches count to lowerBound (lowerBound)
